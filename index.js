...
module.exports = function(app) {
	...
	plugin.start = function(options, restartPlugin) {
		...
		async function savePoint(point) {
			...
			const latitude = parseFloat(point.pos.latitude);
			const longitude = parseFloat(point.pos.longitude);

			if (latitude === 0 && longitude === 0) {
				app.debug(`Skipping recording point with latitude and longitude both equal to 0`);
				return;
			}
			const obj = {
